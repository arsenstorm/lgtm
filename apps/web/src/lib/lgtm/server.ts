...
export const getTodo = createServerFn({ method: 'GET' })
  .validator((id: string) => id)
  .handler(async ({ data }): Promise<TodoDetail> => api<TodoDetail>(`/todos/${data}`))
...
export const getScratchpad = createServerFn({ method: 'GET' })
  .validator((id: string) => id)
  .handler(async ({ data }): Promise<Scratchpad> => api<Scratchpad>(`/scratchpads/${data}`))

export const updateMemory = createServerFn({ method: 'POST' })
  .validator((input: { id: string; content: string }) => input)
  .handler(
    async ({ data }): Promise<Memory> =>
      api<Memory>(`/memories/${data.id}`, { method: 'PATCH', body: { content: data.content } }),
  )

export const deleteMemory = createServerFn({ method: 'POST' })
  .validator((id: string) => id)
  .handler(async ({ data }): Promise<void> => api<void>(`/memories/${data}`, { method: 'DELETE' }))

export const approveMemory = createServerFn({ method: 'POST' })
  .validator((id: string) => id)
  .handler(
    async ({ data }): Promise<Memory> => api<Memory>(`/memories/${data}/approve`, { method: 'POST' }),
  )
...
export const updateTodo = createServerFn({ method: 'POST' })
  ...
  .handler(
    async ({ data }): Promise<Todo> =>
      api<Todo>(`/todos/${data.id}`, { method: 'PATCH', body: data.patch }),
  )
...
export const commentOnTodo = createServerFn({ method: 'POST' })
  ...
  .handler(
    async ({ data }): Promise<TodoComment> =>
      api<TodoComment>(`/todos/${data.id}/comments`, { method: 'POST', body: { body: data.body } }),
  )
...
export const createScratchpad = createServerFn({ method: 'POST' })
  ...
  .handler(
    async ({ data }): Promise<Scratchpad> =>
      api<Scratchpad>('/scratchpads', { method: 'POST', body: data }),
  )

export const updateScratchpad = createServerFn({ method: 'POST' })
  .validator((input: { id: string; content?: string; archived?: boolean }) => input)
  .handler(
    async ({ data }): Promise<Scratchpad> =>
      api<Scratchpad>(`/scratchpads/${data.id}`, {
        ...
        body: { content: data.content, archived: data.archived },
      }),
  )
...
export const deleteScratchpad = createServerFn({ method: 'POST' })
  .validator((id: string) => id)
  .handler(
    async ({ data }): Promise<void> => api<void>(`/scratchpads/${data}`, { method: 'DELETE' }),
  )
